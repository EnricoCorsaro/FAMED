...
A flag specifying whether the rotation test has to be activated. The default value is set to 0, meaning that no rotation is inspected in the individual peaks. We typically recommend to activate this option when analyzing evolved stars, in order to get the most out of the peaks already identified through the detection analysis performed by the CHUNK module.
